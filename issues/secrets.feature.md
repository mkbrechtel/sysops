...
**Module** (`smalltown_devops.secret`) runs on the target. Ensures a secret is provisioned; optionally fetches the value back to the controller for use in subsequent tasks. Arguments:
...
**Lookup plugin** (`smalltown_devops.secret`) runs on the control node and is usable in templates. It does not SSH to the target — it serves the sources/stores that live on the control node (`var`, `ansible-vault`, `local-pass`). For target-resident secrets, use the module with `fetch: true`.

### Example usage

```yaml
- name: Ensure postgres app password
  smalltown_devops.secret:
    service: postgres
