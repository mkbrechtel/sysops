...
func TestPageScrollKeepsCursorVisible(t *testing.T) {
	...
	assertCursorVisible := func(label string) {
		t.Helper()
	...
	for i := 0; i < 400; i++ {
		...
		assertCursorVisible(fmt.Sprintf("pgdown #%d (yOffset=%d)", i, m.viewport.YOffset()))
	...
	for i := 0; i < 400; i++ {
		...
		assertCursorVisible(fmt.Sprintf("pgup #%d (yOffset=%d atEOF=%v)", i, m.viewport.YOffset(), m.atEOF))
	...
	for i := 0; i < 100; i++ {
		m = step(t, m, tea.KeyPressMsg{Code: tea.KeyPgDown})
		assertCursorVisible(fmt.Sprintf("section pgdown #%d (yOffset=%d)", i, m.viewport.YOffset()))
		if m.atEOF {
			break
		}
	}
